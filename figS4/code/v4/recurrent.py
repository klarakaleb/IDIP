...
ETA_I = 0.05 / 1e6  # inhibitory learning rate
A_I = 10  # inhibitory learning amplitude
...
w_rec[:N2E, N2E:] = (
    w_rec[:N2E, N2E:] * 0.15
)  # initial inhibitory weights, instead of 0.1
...
def run(v, w_rec):
    ...
    for i in range(0, LEN_TIME, 1):
        ...
        if t > 15:
            # INHIBITORY LEARNING
            dw = ETA_I * A_I * (i_i - THETA_I) * s[N2E:]
            theta_m[N2E:] = theta_m[N2E:] - dw
    ...
    return w, spikes, i_inputs, thetas


tstart = datetime.datetime.now()

w, spikes, i_inputs, thetas = run(v, w_rec)
