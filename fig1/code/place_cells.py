...
def run(CA1_I_w, CA3_CA1_w):
    ...
    for i in range(0, LEN_TIME * N_RUNS, 1):
        if i % LEN_TIME == 0 and i != 0:
            ...
            np.save(dirname + "/" + str(N_RUNS-1) + "i_input" + filename, e_input[::100])
            np.save(dirname + "/" + str(N_RUNS-1) + "e_input" + filename, i_input[::100])
            refractory_overflow = np.where(t_ref >= LEN_TIME)
            if len(refractory_overflow) != 0:
                t_ref[refractory_overflow] = t_ref[refractory_overflow] % LEN_TIME
            refractory_underflow = np.where(t_ref < LEN_TIME)
        ...
        e_input.append(CA1E_syn_input.copy())
        i_input.append(CA1I_syn_input.copy())
        s[:] = 0

    return w_in, w_i, spikes, i_inputs, i_input, e_input


w_in, w_i, spikes, inputs, i_input, e_input = run(CA1_I_w, CA3_CA1_w)
...
e_input = np.array(e_input)
i_input = np.arrau(i_input)

fr = get_network_firing_rates(spikes.T, 1000, TIME)
np.save(dirname + "/" + str(N_RUNS-1) + "spikes" + filename, spikes)
np.save(dirname + "/" + str(N_RUNS-1) + "fr" + filename, fr[:, 0::100])
np.save(dirname + "/" + str(N_RUNS-1) + "w_i" + filename, w_i[::100])
np.save(dirname + "/" + str(N_RUNS-1) + "w_in" + filename, w_in[::100])
np.save(dirname + "/" + str(N_RUNS-1) + "i_i" + filename, inputs[::100])
np.save(dirname + "/" + str(N_RUNS-1) + "i_input" + filename, e_input[::100])
np.save(dirname + "/" + str(N_RUNS-1) + "e_input" + filename, i_input[::100])
