...
ETA_I = 0.0001  # inhibitory learning rate
A_I = 1  # inhibitory learning amplitude
THETA_I = 0.550  # 000000 * 1e6  # target input for inhibitory neurons
...
def run(v, w_rec):
    ...
    for i in range(0, LEN_TIME, 1):
        ...
        i_input.append(np.sum(syn_c_recurrent_i[:, N2E:], axis=1) * (v + V_I))
        i_i *= EXP_IL
        i_i += np.sum(syn_c_recurrent_i[N2E:, :N2E], axis=1) + np.sum(
            syn_c_input_i[N2E:, :], axis=1
        )
    ...
    return w, spikes, i_inputs, i_input, e_input


tstart = datetime.datetime.now()

w, spikes, i_inputs, i_input, e_input = run(v, w_rec)
...
np.save(path + str(seed) + "_e_input.npy", e_input)
np.save(path + str(seed) + "_i_input.npy", i_input)
