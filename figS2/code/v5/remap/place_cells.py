...
THETA_I = 0.2  # 15
THETA_HOMEO = 5.2
THETA_FR = 80  # target fr for inhibitory neurons
TAU_EST = TAU_M
...
def create_network(N, N1, N1E, N2, N2E):
    n_input_synapses_per_neuron = N1
...
CA1E_current = 0.05 * 0.001
CA1I_current = 0.1 * 0.001
...
"""
input_spikes = np.stack(input_spikes)
...
def run(CA1_I_w, CA3_CA1_w):
    ...
    for i in range(0, LEN_TIME * N_RUNS, 1):
        ...
        fr_est = fr_est + (-fr_est * DT / TAU_EST + s[N2E:])
        ...
        if n_runs > 0:
            # INHIBITORY LEARNING
            delta_w = fr_est / (TAU_EST * 1 / 1000) - THETA_FR
        ...
        fr_estimates.append(fr_est.copy())
        s[:] = 0

    return w_in, w_i, spikes, i_inputs, fr_estimates


CA1_I_w = np.load("../" + str(seed) + "100" + "/w_i_trial.npy")[-1]
CA3_CA1_w = np.load("../" + str(seed) + "100" + "/w_in_trial.npy")[-1]


w_in, w_i, spikes, inputs, fr_estimates = run(CA1_I_w, CA3_CA1_w)
...
np.save(dirname + "/" + "fr" + filename, fr[:, 0::100])
np.save(dirname + "/" + "w_i" + filename, w_i[::100])
np.save(dirname + "/" + "w_in" + filename, w_in[::100])
...
sns.heatmap(fr[:100, ::100], cmap="viridis", vmax=20)
"""
