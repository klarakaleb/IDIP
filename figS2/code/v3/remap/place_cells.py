...
def run(CA1_I_w, CA3_CA1_w):
    ...
    CA1_I_syn_c_i = np.zeros(shape=(N2, (N2 - N2E)))
    ...
    for i in range(0, LEN_TIME * N_RUNS, 1):
        ...
        CA1E_syn_input = np.sum(CA3_CA1_syn_c_i, axis=1) * (v[:N2E] + V_E) - np.sum(
            CA1_I_syn_c_i[:N2E], axis=1
        ) * (v[:N2E] + V_I)
        CA1I_syn_input = np.sum(CA1_E_syn_c_i, axis=1) * (v[N2E:] + V_E) - np.sum(
            CA1_I_syn_c_i[N2E:], axis=1
        ) * (v[N2E:] + V_I)
        i_i = (
            i_i * EXP_IL
            + np.sum(CA1_E_syn_c_i, axis=1)
            - np.sum(CA1_I_syn_c_i[N2E:], axis=1)
        )
        ...
        if n_runs >= 0:
            # INHIBITORY LEARNING
            delta_w = i_i - THETA_I
            w_i_i[:N2E] = w_i_i[:N2E] + (ETA_I * A_I * delta_w) * s[N2E:]
            # EXCITATORY LEARNING
