...
def create_network(n, n1, n1e, n2, n2e, seed, c, c_input, c_i):
    ...
    n_i_per_i_neuron = n_i_per_e_neuron
    ...
    for i in range(0, n2):
        ...
        if i >= n2e:
            ...
            synapse_type_mat[excitatory_inputs, i] = 1
            inhibitory_inputs = np.random.choice(
                range(n2e, n2), size=n_i_per_i_neuron, replace=False
            )
            wrec[inhibitory_inputs, i] = 1
            synapse_type_mat[inhibitory_inputs, i] = -1
            # avoid self connections
            SELF = False
            if i in inhibitory_inputs:
                SELF = True
            while SELF:
                where = np.where(inhibitory_inputs == i + n1)
                # sample from the rest to avoid duplicate connections
                inhibitory_inputs[where] = np.random.choice(
                    np.array([i for i in range(n2e, n2) if i not in inhibitory_inputs]),
                    size=len(where),
                    replace=False,
                )
                if i not in inhibitory_inputs[where]:
                    SELF = False
...
w_rec[:, N2E:] = w_rec[:, N2E:] * 0.1  # initial inhibitory weights
...
def run(v, w_rec):
    ...
    for i in range(0, LEN_TIME, 1):
        ...
        i_i *= EXP_IL
        i_i += (
            np.sum(syn_c_recurrent_i[N2E:, :N2E], axis=1)
            + np.sum(syn_c_input_i[N2E:, :], axis=1)
            - np.sum(syn_c_recurrent_i[N2E:, N2E:], axis=1)
        )
